...
export const handler = async (event) => {
  ...
  try {
    ...
    if (path === '/register' && event.httpMethod === 'POST') {
      ...
      if (!name || !email || !password) return cors({ error: 'Faltan datos por completar' }, 400);
      ...
      if (existing) return cors({ error: 'Este email ya está registrado' }, 409);
    ...
    if (path === '/login' && event.httpMethod === 'POST') {
      ...
      if (!email || !password) return cors({ error: 'Faltan datos por completar' }, 400);

      const { data: user } = await supabase
        .from('users').select('*').eq('email', email.toLowerCase()).maybeSingle();
      if (!user) return cors({ error: 'Email o contraseña incorrectos' }, 401);

      const valid = bcrypt.compareSync(password, user.password);
      if (!valid) return cors({ error: 'Email o contraseña incorrectos' }, 401);
